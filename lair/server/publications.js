        ...
        var hostHandle = Hosts.find(hostQuery).observeChanges({
            added: function (id) {
                ...
                if (!initializing) {
                    ...
                    self.changed('counts', projectId, {
                        hostLairGrey: hostStatus['lair-grey']
                    });
                    self.changed('counts', projectId, {
                        hostLairBlue: hostStatus['lair-blue']
                    });
                    self.changed('counts', projectId, {
                        hostLairGreen: hostStatus['lair-green']
                    });
                    self.changed('counts', projectId, {
                        hostLairOrange: hostStatus['lair-orange']
                    });
                    self.changed('counts', projectId, {
                        hostLairRed: hostStatus['lair-red']
                    });
                }
            },
            removed: function (id) {
                ...
                hostStatus['lair-grey'] = Hosts.find({
                    'project_id': projectId,
                    'status': 'lair-grey'
                }).count();
                hostStatus['lair-blue'] = Hosts.find({
                    'project_id': projectId,
                    'status': 'lair-blue'
                }).count();
                hostStatus['lair-green'] = Hosts.find({
                    'project_id': projectId,
                    'status': 'lair-green'
                }).count();
                hostStatus['lair-orange'] = Hosts.find({
                    'project_id': projectId,
                    'status': 'lair-orange'
                }).count();
                hostStatus['lair-red'] = Hosts.find({
                    'project_id': projectId,
                    'status': 'lair-red'
                }).count();
                self.changed('counts', projectId, {
        ...
        var vulnHandle = Vulnerabilities.find(vulnQuery).observeChanges({
            added: function (id) {
                ...
                if (!initializing) {
                    ...
                    self.changed('counts', projectId, {
                        vulnLairGrey: vulnStatus['lair-grey']
                    });
                    self.changed('counts', projectId, {
                        vulnLairBlue: vulnStatus['lair-blue']
                    });
                    self.changed('counts', projectId, {
                        vulnLairGreen: vulnStatus['lair-green']
                    });
                    self.changed('counts', projectId, {
                        vulnLairOrange: vulnStatus['lair-orange']
                    });
                    self.changed('counts', projectId, {
                        vulnLairRed: vulnStatus['lair-red']
                    });
                }
            },
            removed: function (id) {
                ...
                vulnStatus['lair-grey'] = Vulnerabilities.find({
                    'project_id': projectId,
                    'status': 'lair-grey'
                }).count();
                vulnStatus['lair-blue'] = Vulnerabilities.find({
                    'project_id': projectId,
                    'status': 'lair-blue'
                }).count();
                vulnStatus['lair-green'] = Vulnerabilities.find({
                    'project_id': projectId,
                    'status': 'lair-green'
                }).count();
                vulnStatus['lair-orange'] = Vulnerabilities.find({
                    'project_id': projectId,
                    'status': 'lair-orange'
                }).count();
                vulnStatus['lair-red'] = Vulnerabilities.find({
                    'project_id': projectId,
                    'status': 'lair-red'
                }).count();
                self.changed('counts', projectId, {
...
Meteor.publish('vulnerabilityHosts', function (projectId, vulnerabilityId) {
    ...
    if (project && (project.owner == this.userId || _.indexOf(project.contributors, this.userId) > -1)) {
        ...
        return Hosts.find({
            'project_id': projectId,
            'string_addr': {
                '$in': _.pluck(vuln.hosts, 'string_addr')
            }
        });
    }
    return false;
